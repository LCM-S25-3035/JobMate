...
def calculate_enhanced_ats_score(resume_text, keywords=None, job_description=""):
    """Enhanced ATS scoring with GUARANTEED 90+ for optimized resumes"""
    try:
        # Use the new enhanced analyzer if available
        from app.services.ats_analyzer import EnhancedATSAnalyzer
        analyzer = EnhancedATSAnalyzer()
        
        if keywords is None:
            keywords = analyzer.extract_job_keywords(job_description)
        
        score, breakdown = analyzer.calculate_comprehensive_ats_score(
            resume_text, job_description, keywords
        )
        
        current_app.logger.info(f"Enhanced ATS Score: {score}% (using new analyzer)")
        return score
        
    except ImportError:
        current_app.logger.warning("Enhanced ATS analyzer not available, using legacy scoring")
        # Fallback to existing legacy algorithm
        return calculate_enhanced_ats_score_legacy(resume_text, keywords, job_description)
    except Exception as e:
        current_app.logger.error(f"Error in enhanced ATS scoring: {str(e)}")
        return calculate_enhanced_ats_score_legacy(resume_text, keywords, job_description)


def calculate_enhanced_ats_score_legacy(resume_text, keywords, job_description):
    """Legacy enhanced ATS scoring with GUARANTEED 90+ for optimized resumes"""
    score = 0
...
def remove_duplicate_section_headers(resume_text):
    """Remove duplicate section headers (SUMMARY, SKILLS, EXPERIENCE, EDUCATION) from resume and merge content"""
    lines = resume_text.split('\n')
    sections = {}
    current_section = None
    header_order = []
    
    # First pass: collect all content under section headers
    for line in lines:
        line_stripped = line.strip()
        line_upper = line_stripped.upper()
        
        # Check if this is a section header
        if line_upper in ['SUMMARY', 'PROFESSIONAL SUMMARY', 'SKILLS', 'TECHNICAL SKILLS', 
                         'EXPERIENCE', 'WORK EXPERIENCE', 'EDUCATION', 'EDUCATIONAL BACKGROUND']:
            # Normalize header to standard format
            if line_upper in ['SUMMARY', 'PROFESSIONAL SUMMARY']:
                normalized_header = 'SUMMARY'
            elif line_upper in ['SKILLS', 'TECHNICAL SKILLS']:
                normalized_header = 'SKILLS'
            elif line_upper in ['EXPERIENCE', 'WORK EXPERIENCE']:
                normalized_header = 'EXPERIENCE'
            elif line_upper in ['EDUCATION', 'EDUCATIONAL BACKGROUND']:
                normalized_header = 'EDUCATION'
            else:
                normalized_header = line_upper
            
            # Track order of first occurrence
            if normalized_header not in sections:
                header_order.append(normalized_header)
            ...
            # Content before any section (usually contact info)
            if 'HEADER' not in sections:
                header_order.insert(0, 'HEADER')
                sections['HEADER'] = []
            sections['HEADER'].append(line)
    
    # Second pass: reconstruct resume with merged content
    clean_lines = []
    
    for section_name in header_order:
        if section_name == 'HEADER':
            # Add header content (contact info, etc.)
            clean_lines.extend(sections[section_name])
        else:
            # Add section header
            clean_lines.append(section_name)
            ...
            for content_line in content_lines:
                ...
                elif not content_stripped:  # Keep empty lines for formatting
                    merged_content.append(content_line)
            
            clean_lines.extend(merged_content)
...
@bp.route('/mark-applied/<job_id>', methods=['POST'])
@login_required
def mark_as_applied(job_id):
    """Mark a job as applied when user applied externally"""
    try:
        current_app.logger.info(f"🔄 Mark as applied request for job_id: {job_id} by user: {current_user.id}")
        
        # Check if user is authenticated and is an applicant
        if not current_user.is_authenticated or current_user.user_type != 'applicant':
            current_app.logger.error(f"❌ Unauthorized access attempt by user: {current_user.id}")
            return jsonify({'success': False, 'error': 'Unauthorized access'}), 403
        
        mongo_db = get_mongo_db()
        if mongo_db is None:
            current_app.logger.error("❌ MongoDB connection is None")
            return jsonify({'success': False, 'error': 'Database connection error'}), 500
        ...
        try:
            if job_id.startswith('recruiter_'):
                current_app.logger.info(f"📊 Processing recruiter job: {job_id}")
                # This is a PostgreSQL recruiter job
                from app.models.job_posting import JobPosting
                pg_job_id = job_id.replace('recruiter_', '')
                
                try:
                    pg_job_id_int = int(pg_job_id)
                    pg_job = JobPosting.query.filter_by(id=pg_job_id_int, status='active').first()
                except ValueError:
                    current_app.logger.error(f"❌ Invalid recruiter job ID format: {pg_job_id}")
                ...
                if pg_job:
                    ...
                    current_app.logger.info(f"✅ Found recruiter job: {pg_job.title}")
                else:
                    current_app.logger.warning(f"⚠️ Recruiter job not found: {pg_job_id}")
            else:
                current_app.logger.info(f"🌐 Processing MongoDB job: {job_id}")
                # This is a MongoDB external job
                try:
                    from bson.errors import InvalidId
                    object_id = ObjectId(job_id)
                    job = mongo_db.jobs.find_one({'_id': object_id})
                    if job:
                        job['is_recruiter_job'] = False
                        current_app.logger.info(f"✅ Found MongoDB job: {job.get('title', 'Unknown')}")
                    else:
                        current_app.logger.warning(f"⚠️ MongoDB job not found: {job_id}")
                except InvalidId:
                    current_app.logger.error(f"❌ Invalid MongoDB ObjectId format: {job_id}")
                    return jsonify({'success': False, 'error': 'Invalid job ID format'}), 400
                except Exception as oid_error:
                    current_app.logger.error(f"❌ Error processing MongoDB job {job_id}: {str(oid_error)}")
                    return jsonify({'success': False, 'error': f'Database error: {str(oid_error)}'}), 500
                        
        except Exception as e:
            current_app.logger.error(f"❌ Error fetching job {job_id}: {str(e)}")
            return jsonify({'success': False, 'error': f'Error fetching job: {str(e)}'}), 500
        
        if not job:
            current_app.logger.error(f"❌ Job not found: {job_id}")
            return jsonify({'success': False, 'error': 'Job not found'}), 404
        
        current_app.logger.info(f"✅ Job found: {job.get('title', 'Unknown')} at {job.get('company', 'Unknown')}")
        ...
        if mongo_db is not None:
            ...
            if existing_application:
                current_app.logger.info(f"⚠️ User already applied in MongoDB: {job_id}")
        
        # Also check PostgreSQL for recruiter jobs
        if not existing_application and job.get('is_recruiter_job'):
            from app.models.application import Application
            pg_existing = Application.query.filter_by(
                user_id=current_user.id,
                job_posting_id=job.get('job_posting_id')
            ).first()
            if pg_existing:
                existing_application = True
                current_app.logger.info(f"⚠️ User already applied in PostgreSQL: {job_id}")
        
        if existing_application:
            return jsonify({'success': False, 'error': 'You have already applied to this job'}), 400
        
        current_app.logger.info(f"🔄 Creating application record for user {current_user.id}")
        
        # Create job application record for external application
        from datetime import datetime
        application_data = {
            'user_id': str(current_user.id),
            'job_id': job_id,
            ...
            'applied_at': datetime.utcnow(),
            'status': 'applied',
            'method': 'external_manual',  # Indicate this was manually marked as applied
        ...
        try:
            if mongo_db is not None:
                application_result = mongo_db.job_applications.insert_one(application_data)
                current_app.logger.info(f"✅ MongoDB application created: {application_result.inserted_id}")
            else:
                current_app.logger.error("❌ MongoDB connection is None during application creation")
        except Exception as mongo_error:
            current_app.logger.error(f"❌ Failed to create MongoDB application: {str(mongo_error)}")
            # Continue with PostgreSQL creation if this is a recruiter job
        ...
        if job.get('is_recruiter_job'):
            try:
                ...
                current_app.logger.info(f"🔄 Creating PostgreSQL application for recruiter job")
                ...
                pg_application = Application(
                    ...
                    applied_at=datetime.utcnow(),
                    cover_letter='',
                    notes='Manually marked as applied (external application)',
                ...
                current_app.logger.info(f"✅ SUCCESS: Created PostgreSQL application {pg_application_id} for manual application to job {job_id}")
                
            except Exception as e:
                current_app.logger.error(f"❌ ERROR: Failed to create PostgreSQL application: {str(e)}")
                db.session.rollback()
                # If both MongoDB and PostgreSQL fail, return error
                if not application_result:
                    return jsonify({'success': False, 'error': 'Failed to create application record'}), 500
        
        # Save to Talent Journey
        if mongo_db is not None:
            try:
                talent_journey_entry = {
                    'user_id': str(current_user.id),
                    ...
                    'company': job.get('company', ''),
                    'ats_score': None,
                    'application_method': 'external_manual',
                    'application_id': str(application_result.inserted_id) if application_result else None,
                    'pg_application_id': pg_application_id,
                    'details': {
                        'applied_externally': True,
                        'manually_tracked': True,
                        'notes': 'User manually marked job as applied'
                    }
                }
                
                mongo_db.talent_journey.insert_one(talent_journey_entry)
                current_app.logger.info(f"✅ SUCCESS: Added talent journey entry for manual application to {job.get('title')}")
        ...
        # Validate that at least one application was created
        if not application_id:
            current_app.logger.error(f"❌ CRITICAL: No application was created for job {job_id}")
            return jsonify({'success': False, 'error': 'Failed to create application record'}), 500
        
        current_app.logger.info(f"🎉 SUCCESS: Application created with ID {application_id}")
        
        return jsonify({
            'success': True,
            'message': f'Successfully marked "{job.get("title", "job")}" at {job.get("company", "company")} as applied!',
            'application_id': application_id,
            'manually_tracked': True
        })
        
    except Exception as e:
        current_app.logger.error(f"❌ CRITICAL ERROR in mark_as_applied for job {job_id}: {str(e)}")
        current_app.logger.error(f"❌ Exception type: {type(e).__name__}")
        current_app.logger.error(f"❌ User: {current_user.id if current_user.is_authenticated else 'Not authenticated'}")
        import traceback
...
def download_tailored_resume_strict(job_id):
    ...
    try:
        ...
        # ALWAYS apply duplicate removal first
        current_app.logger.info("Applying duplicate section header removal")
        clean_content = remove_duplicate_section_headers(resume_content)
        ...
        if not has_skills_section or not has_experience_section or not has_summary_section:
            ...
            clean_content = master_resume_formatter(clean_content)
...
**Note:** Detailed job description is not available in our system. Please visit the company's website or the original job posting for complete details."""
    ...
    if current_user.is_authenticated:
        ...
        if mongo_db is not None:
            ...
            # Also check PostgreSQL for recruiter jobs
            if not has_applied and job.get('is_recruiter_job') and job.get('job_posting_id'):
                from app.models.application import Application
                pg_application = Application.query.filter_by(
                    user_id=current_user.id,
                    job_posting_id=job.get('job_posting_id')
                ).first()
                has_applied = bool(pg_application)
